...
This package is a collection of python based tools to help with visualising and analysing Grazing Incidence Wide Angle X-ray Scattering (GIWAXS) data collected at Diamond Light Source on beamline I07. 

The initial processing and mapping step is handled by the `fast_rsm`_ package, and this toolbox is designed to work with the output from that package. The toolbox includes a collection of interactive widgets to help explore and visualise GIWAXS datasets, as well as tools for extracting datasets for further analysis including fitting peaks in 1D datasets.



Accessing the giwaxs_toolbox
-------------------------------

Option 1:
****************

    `diamond_guide`_ : Guide on accessing the giwaxs_toolbox on the Diamond filesystem 

Option 2:
****************
   `local_install`_ : If you are familiar with setting up and using python environments you can use the following instructions for installing a local version of giwaxs_toolbox



Guides on using the giwaxs_toolbox
-----------------------------------------

   `walkthrough guides`_ : Walkthroughs with examples on using the various options available within the giwaxs_toolbox
...
.. _diamond_guide: ./diamond_guide.html
.. _local_install: ./local_install.html
